				...
				*/
				...
				$output .='<div class="hgr_blog_posts ' . esc_attr( $css_class ) . '" id="'.$uniqueID.'" data-fetch="'.$posts_number.'" data-cols="'.$posts_columns.'">';
					...
					while ( $hgr_query->have_posts() ) {
						...
						$hgr_query->the_post();
						$output .='<div id="hgr_blog_post_'.mt_rand(999, 9999999).'" class="hgr_blog_post '.(!empty($extra_class) ? $extra_class : '').'">';
								...
								case 'img_title_txt':
									...
									elseif($blogpost_footer == 'compact') {
										...
										$output .='<div class="hgr_post_metas_simple">';
											$output .='<div class="hgr_post_meta_simple">@ ' . esc_html__('by', 'hgrextender').' '.ucwords(get_the_author()).'</div>';
										$output .='</div>';
									}
								break;
