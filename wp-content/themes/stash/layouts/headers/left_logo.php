    ...
    <div id="hgr_top_navbar_extras">
    	...
    	<?php 
		  ...
		  <div class="hgr_woo_minicart sage-cart-icon <?php echo esc_attr($hideonmobile);?>"><span class="helper"></span>
			<div class="woo_bubble"><a class="hgr_woo_minicart_content" href="<?php global $woocommerce; echo esc_url($woocommerce->cart->get_cart_url()); ?>" title="<?php esc_html_e('View your shopping cart', 'stash'); ?>">
			<?php echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'stash'), $woocommerce->cart->cart_contents_count);?></a><?php echo ( class_exists('HGR_QCV') ? do_shortcode( '[hgr_quick_cart]' ) : '' );?></div>
		  </div>
