<?php
/*
 * @package Stash WordPress Theme by HighGrade
 * @since version 1.0
 */

 // Add multilanguage support
load_theme_textdomain( 'stash', get_template_directory() . '/languages' );


 // Enqueue CSS and JS script to frontend header and footer
 if( !function_exists('stash_enqueue') ) {
	function stash_enqueue() {
		$hgr_options = get_option( 'redux_options' );

		// CSS
		wp_enqueue_style( 'stash_icons',
		...
		wp_enqueue_style( 'stash_css_component',
		...
		wp_enqueue_style( 'lity',
			trailingslashit( get_template_directory_uri() ) . 'highgrade/css/lity.min.css',
			'',
			''
		);

		wp_enqueue_style( 'stash_style', get_stylesheet_uri() );

		if ( !class_exists( 'ReduxFramework' ) ){
			wp_enqueue_style( 'noredux',
			trailingslashit( get_template_directory_uri() ) . 'highgrade/css/noredux.css',
			'',
			''
		);
		}
		...
		if( !class_exists('Vc_Manager') ){
			wp_enqueue_script( 'isotope',
				trailingslashit( get_template_directory_uri() ) . 'highgrade/js/isotope.pkgd.min.js',
				array('jquery'),
				'',
				true
			);
		...
		wp_enqueue_script( 'stash_modernizr_custom',
		...
		wp_enqueue_script( 'lity',
			trailingslashit( get_template_directory_uri() ) . 'highgrade/js/lity.min.js',
			array('jquery'),
			'',
			true
		);

		wp_enqueue_script( 'stash_colors',
		...
		wp_enqueue_script( 'stash_velocity',
		...
		if( is_array($hgr_options) && isset( $hgr_options['enable_smooth_scroll'] ) && esc_attr( $hgr_options['enable_smooth_scroll'] ) == 1 ) {
			wp_enqueue_script(	'stash_smoothscroll',
		...
		$php_variables_array = array(
			...
			'static_mega_footer'		=> ( isset($hgr_options['enable_full_static_megafooter']) && $hgr_options['enable_full_static_megafooter'] == true ? 'true' : 'false' ),
			'metro_asprect_ratio'		=>	( isset($hgr_options['blog_metro_asprect_ratio']) && !empty($hgr_options['blog_metro_asprect_ratio']) ? $hgr_options['blog_metro_asprect_ratio'] : '1x1' )
		);

		wp_localize_script( 'stash_js', 'php_variables', $php_variables_array );

		// Add custom js to main js
		$custom_js_tba = ( isset( $hgr_options['enable_js-code'] ) && $hgr_options['enable_js-code'] == 'custom_js_on' ? $hgr_options['js-code'] : "''" );
		if(!empty($custom_js_tba)){
			wp_add_inline_script('stash_js', $custom_js_tba );
		}
		...
		if( !class_exists('Vc_Manager') ){
			wp_enqueue_style( 'ekko_nojscomposer',
			trailingslashit( get_template_directory_uri() ) . 'highgrade/css/nojscomposer.css',
			'',
			''
		);
		}
 ...
 add_action( 'wp_enqueue_scripts', 'stash_enqueue' );


add_action( 'admin_enqueue_scripts', 'stash_admin_enqueue' );
function stash_admin_enqueue() {
...
*/
add_filter( 'body_class', 'stash_add_bodyClass' );
function stash_add_bodyClass( $classes ) {
 ...
 if( !function_exists('stash_pagination') ) {
	function stash_pagination( $args = '' ) {
		$defaults = array(
			'before' => '<p id="post-pagination">' . esc_html__( 'Pages:', 'stash' ),
			'after' => '</p>',
			'text_before' => '',
			'text_after' => '',
			'next_or_number' => 'number',
			'nextpagelink' => esc_html__( 'Next page', 'stash' ),
			'previouspagelink' => esc_html__( 'Previous page', 'stash' ),
			'pagelink' => '%',
			'echo' => 0
		);
...
*	Custom hooks
*/
function stash_after_body_open(){ do_action( 'stash_after_body_open' ); }
function stash_before_footer_open(){ do_action( 'stash_before_footer_open' ); }
...
if( !function_exists('stash_register_required_plugins') ) {
...
function stash_add_editor_styles() {
    add_editor_style( 'custom-editor-style.css' );
}
add_action( 'admin_init', 'stash_add_editor_styles' );


 // Some basic setup after theme setup
 add_action( 'after_setup_theme', 'stash_theme_setup' );
 function stash_theme_setup(){
...
add_action('after_switch_theme', 'stash_theme_change');
function stash_theme_change () {
	...
	if( $hgr_options && !is_array($hgr_options) ){
		...
		$json_content = '{"last_tab":"","website_model":"website_full_width","enable_smooth_scroll":"0","enable_boxed_shadow":"0","website-background":{"background-color":"#f2f2f2","background-repeat":"repeat","background-size":"cover","background-attachment":"fixed","background-position":"center center","background-image":"","media":{"id":"","height":"","width":"","thumbnail":""}},"enable_full_screen_search":"1","enable_fssearch_onlu_for_products":"0","body_border":"body_border_off","body_border_dimensions":{"width":"20px","units":"px"},"body_border_color":{"color":"#ffffff","alpha":"1","rgba":"rgba(255,255,255,1)"},"custom_error_page":"","back_to_top_button":"0","back_to_top_button_bg_color":{"color":"#dd9933","alpha":"1.0","rgba":"rgba(221,153,51,1)"},"back_to_top_button_dimensions":{"width":"30px","units":"px"},"mediaquery_screen_xs":"480","mediaquery_screen_s":"640","mediaquery_screen_m":"768","mediaquery_screen_l":"980","mediaquery_screen_xl":"1280","mediaquery_screen_xxl":"1400","container_xs":{"width":"300px","units":"px"},"container_s":{"width":"440px","units":"px"},"container_m":{"width":"600px","units":"px"},"container_l":{"width":"720px","units":"px"},"container_xl":{"width":"920px","units":"px"},"container_xxl":{"width":"1100px","units":"px"},"container_over_xxl":{"width":"1280px","units":"px"},"logo":{"url":"","id":"","height":"","width":"","thumbnail":""},"retina_logo":{"url":"","id":"","height":"","width":"","thumbnail":""},"favicon":{"url":"","id":"","height":"","width":"","thumbnail":""},"bg_color":{"color":"#ffffff","alpha":"1","rgba":"rgba(255,255,255,1)"},"theme_dominant_color":"#1c46f2","ds_text_color":"#dcdff2","h1_color":"#ffffff","h2_color":"#ffffff","h3_color":"#dcdff2","h4_color":"#ffffff","h5_color":"#ffffff","h6_color":"#ffffff","ahref_color":{"regular":"#dcdff2","hover":"#ffffff"},"ls_text_color":"#4b555f","light_h1_color":"#292929","light_h2_color":"#292929","light_h3_color":"#292929","light_h4_color":"#292929","light_h5_color":"#1c46f2","light_h6_color":"#292929","light_ahref_color":{"regular":"#47535e","hover":"#1c46f2"},"body-font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"400","font-style":"","subsets":"latin","text-align":"","text-transform":"","font-size":"15px","line-height":"28px","letter-spacing":"0.2px"},"h1-font":{"font-family":"Montserrat","font-options":"","google":"true","font-weight":"400","font-style":"","subsets":"latin","text-align":"","text-transform":"","font-size":"36px","line-height":"48px","letter-spacing":""},"h2-font":{"font-family":"Montserrat","font-options":"","google":"true","font-weight":"400","font-style":"","subsets":"latin","text-align":"","text-transform":"","font-size":"30px","line-height":"42px","letter-spacing":"0px"},"h3-font":{"font-family":"Montserrat","font-options":"","google":"true","font-weight":"400","font-style":"","subsets":"latin","text-align":"","text-transform":"","font-size":"24px","line-height":"36px","letter-spacing":"0px"},"h4-font":{"font-family":"Montserrat","font-options":"","google":"true","font-weight":"400","font-style":"","subsets":"latin","text-align":"","text-transform":"capitalize","font-size":"18px","line-height":"30px","letter-spacing":""},"h5-font":{"font-family":"Montserrat","font-options":"","google":"true","font-weight":"400","font-style":"","subsets":"latin","text-align":"","text-transform":"none","font-size":"14px","line-height":"24px","letter-spacing":""},"h6-font":{"font-family":"Montserrat","font-options":"","google":"true","font-weight":"400","font-style":"","subsets":"latin","text-align":"","text-transform":"none","font-size":"12px","line-height":"18px","letter-spacing":""},"enable_page_title":"1","page_title_h1":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"700","font-style":"","subsets":"latin","text-align":"center","text-transform":"none","font-size":"60px","line-height":"72px","letter-spacing":"-1px","color":"#000000"},"page_title_padding":{"padding-top":"50px","padding-right":"0","padding-bottom":"50px","padding-left":"0","units":"px"},"typekit_embed_code":"","typekit_custom_css":"#main_navbar li a, .esg-grid, .wpcf7-submit, .wpcf7 input, .wpcf7 textarea, .hgr_mc_collector, button, .blog a, p, h1, h2, h3, h4, h5, h6{\r\nfont-family: \"soleil\" !important;\r\n}","menu_bar_width":"menu_contained","menu-font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"","font-style":"","subsets":"latin","text-align":"","text-transform":"uppercase","font-size":"12px","line-height":"60px","letter-spacing":"1px"},"menu-font-hover-color":{"regular":"#292929","hover":"#1c46f2"},"identity_padding":{"padding-top":"5px","padding-right":"0","padding-bottom":"5px","padding-left":"0","units":"px"},"top_menu_padding":{"padding-top":"5px","padding-right":"0","padding-bottom":"5px","padding-left":"0","units":"px"},"top_middle_bar_right_side_padding":{"padding-top":"","padding-right":"","padding-bottom":"","padding-left":"","units":"px"},"header_floating":"1","header_floating_display_after":{"height":"200px","units":"px"},"header_floating_hide_after":{"height":"200px","units":"px"},"header_shrink_after_scroll":{"height":"200px","units":"px"},"menu_bar_initial_height":{"height":"150px","units":"px"},"menu_bar_final_height":{"height":"100px","units":"px"},"header_transparent_display_after":{"height":"200px","units":"px"},"header_transp_bg_opacity_after_scroll":"1.0","header_background_type":"1","header_background_rgba":{"color":"#ffffff","alpha":"1","rgba":"rgba(255,255,255,1)"},"header_transparent_bg_rgba":{"color":"#ffffff","alpha":"1.0","rgba":"rgba(255,255,255,1)"},"header_background_image":{"background-repeat":"","background-size":"","background-attachment":"","background-position":"","background-image":"","media":{"id":"","height":"","width":"","thumbnail":""}},"header_opacity_change_after_scroll":"2","header_background_opacity_change_after_amount":{"height":"200px","units":"px"},"header_background_opacity_after_scroll":"0.5","header_border":{"border-top":"","border-right":"","border-bottom":"","border-left":"","border-style":"none","border-color":"#f2f2f2"},"header_margins":{"margin-top":"0","margin-right":"0","margin-bottom":"0","margin-left":"0","units":"px"},"menu_bar_padding":{"padding-top":"0","padding-right":"30px","padding-bottom":"0","padding-left":"30px","units":"px"},"page_top_offset":{"height":"0px","units":"px"},"fixed_menu-font_top_bar":{"font-family":"Roboto","font-options":"","google":"1","font-weight":"400","font-style":"","subsets":"","text-align":"","text-transform":"","font-size":"12px","line-height":"36px","letter-spacing":"","color":"#ffffff"},"fixed_menu-font-hover-color_top_bar":{"regular":"#2be2ff","hover":"#ffffff"},"top_bar_background_rgba":{"color":"#2a2a2a","alpha":"1","rgba":"rgba(42,42,42,1)"},"top_bar_left_column":"<li class=\"fa fa-map-marker\" style=\"font-size:14px\"></li> 51 Brandywine Drive, Ridgecrest, CA 93555. Locate us on the <a href=\"http://highgradelab.com/cast/main-demo/contact/\">map</a>.","top_bar_padding":{"padding-top":"","padding-right":"","padding-bottom":"","padding-left":"","units":"px"},"fixed_menu-font_middle_bar":{"font-family":"Roboto","font-options":"","google":"1","font-weight":"400","font-style":"","subsets":"","text-align":"","text-transform":"","font-size":"14px","line-height":"18px","letter-spacing":"","color":"#2a2a2a"},"fixed_menu-font-hover-color_middle_bar":{"regular":"#000000","hover":"#dd9933"},"middle_bar_background_rgba":{"color":"#ffffff","alpha":"1.0","rgba":"rgba(255,255,255,1)"},"middle_bar_first_column":"<p>+1-202-555-0170</br>\r\n<strong><li class=\"fa fa-phone\" style=\"font-size:14px\"></li> Toll Free Call</strong></p>","middle_bar_second_column":"<p>office@cast.com<br>\r\n<strong><li class=\"fa fa-envelope\" style=\"font-size:14px\"></li> Office Email</strong></p>","middle_bar_third_column":"<p>M - F: 10:00 - 18:00<br>\r\n<strong><li class=\"fa fa-clock-o\" style=\"font-size:14px\"></li> Office Hours</strong></p>","middle_bar_padding":{"padding-top":"15px","padding-right":"0","padding-bottom":"15px","padding-left":"0","units":"px"},"fixed_menu-font":{"font-family":"Roboto","font-options":"","google":"1","font-weight":"500","font-style":"","subsets":"","text-align":"","text-transform":"","font-size":"14px","line-height":"60px","letter-spacing":""},"fixed_menu-font-hover-color":{"regular":"#2a2a2a","hover":"#ffffff"},"bottom_bar_background_rgba":{"color":"#2be2ff","alpha":"1","rgba":"rgba(43,226,255,1)"},"bottom_bar_padding":{"padding-top":"10px","padding-right":"0","padding-bottom":"10px","padding-left":"0","units":"px"},"mobile-menu-font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"500","font-style":"","subsets":"","text-align":"left","text-transform":"uppercase","font-size":"14px","line-height":"60px","letter-spacing":"1px"},"mobile-menu-font-hover-color":{"regular":"#292929","hover":"#1c46f2"},"mobile_menu_background_rgba":{"color":"#ffffff","alpha":"1","rgba":"rgba(255,255,255,1)"},"footer-bgcolor":"#222222","footer_color_scheme":"dark-scheme","footer-copyright":"Copyright 2017 <a href=\"http://www.highgradelab.com\">HighGrade</a>. All rights reserved.","hgr_megafooter_select":"","cf_label_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"400","font-style":"","subsets":"","text-align":"left","text-transform":"none","font-size":"12px","line-height":"18px","color":"#4b555f"},"cf_input_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"500","font-style":"","subsets":"latin","text-align":"","text-transform":"","font-size":"13px","line-height":"18px","color":"#4b555f"},"cf_input_field_bg":{"color":"#ffffff","alpha":"1","rgba":"rgba(255,255,255,1)"},"cf_input_field_roundness":{"padding-top":"0","padding-right":"0","padding-bottom":"0","padding-left":"0","units":"px"},"cf_input_padding":{"padding-top":"15px","padding-right":"20px","padding-bottom":"15px","padding-left":"20px","units":"px"},"cf_input_margin":{"margin-top":"0","margin-right":"0","margin-bottom":"10px","margin-left":"0","units":"px"},"cf_input_border":{"border-top":"2px","border-right":"2px","border-bottom":"2px","border-left":"2px","border-style":"solid","border-color":"#e9ecef"},"cf_input_submit_height":{"height":"48px","units":"px"},"cf_input_submit_bg":{"color":"#1c46f2","alpha":"1","rgba":"rgba(28,70,242,1)"},"cf_input_submit_hover_bg":{"color":"#1d43dd","alpha":"1","rgba":"rgba(29,67,221,1)"},"cf_input_submit_clr":{"regular":"#ffffff","hover":"#ffffff"},"woo_support":"1","products_per_row":"4","shop_body_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"400","font-style":"","subsets":"latin","text-align":"left","text-transform":"none","font-size":"15px","line-height":"28px","color":"#4b555f"},"shop_h3_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"","font-style":"","subsets":"latin","text-align":"left","text-transform":"none","font-size":"18px","line-height":"24px","color":"#292929"},"shop_price_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"","font-style":"","subsets":"latin","text-align":"left","text-transform":"capitalize","font-size":"14px","line-height":"18px","color":"#1c46f2"},"shop_h1_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"","font-style":"","subsets":"latin","text-align":"left","text-transform":"none","font-size":"36px","line-height":"48px","color":"#292929"},"shop_single_price_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"","font-style":"","subsets":"latin","text-align":"left","text-transform":"none","font-size":"24px","line-height":"36px","color":"#1c46f2"},"shop_h4_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"","font-style":"","subsets":"latin","text-align":"left","text-transform":"none","font-size":"18px","line-height":"30px","color":"#292929"},"shop_ahref_color":{"regular":"#292929","hover":"#1c46f2"},"shop_bg_color":"#ffffff","woo_addtocart_btn_color":{"regular":"#000000","hover":"#000000"},"woo_addtocart_btn_text_color":{"regular":"#ffffff","hover":"#ffffff"},"woo_addtocart_btn_text_font":{"font-family":"Roboto","font-options":"","google":"1","font-weight":"700","font-style":"","subsets":"","text-align":"","text-transform":"uppercase","font-size":"12px","line-height":"25px","letter-spacing":"2px"},"qcv_chk_btn_bg_color":{"regular":"#1c46f2","hover":"#1c46f2"},"qcv_btns":{"font-family":"Roboto","font-options":"","google":"1","font-weight":"700","font-style":"","subsets":"","text-align":"center","text-transform":"","font-size":"14px","line-height":"30px","color":"#ffffff"},"custom_empty_cart":"","blog_type":"blog_basic","blog_grid_columns":"4","blog_grid_image":"blog_grid_image_on","blog_metro_columns":"4","blog_metro_columns_gap":{"width":"20px","units":"px"},"blog_metro_asprect_ratio":"1x1","blog_list_image":"blog_list_image_on","blog_list_image_alternate":"","blog_sidebar":"sidebar_right","blog_body_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"400","font-style":"","subsets":"latin","text-align":"","text-transform":"","font-size":"15px","line-height":"28px","color":"#4b555f"},"blog_h1_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"","font-style":"","subsets":"latin","text-align":"","text-transform":"","font-size":"36px","line-height":"48px","color":"#292929"},"blog_h2_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"","font-style":"","subsets":"latin","text-align":"","text-transform":"","font-size":"30px","line-height":"42px","color":"#292929"},"blog_h3_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"400","font-style":"","subsets":"latin","text-align":"","text-transform":"","font-size":"24px","line-height":"36px","color":"#292929"},"blog_h4_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"","font-style":"","subsets":"latin","text-align":"","text-transform":"","font-size":"18px","line-height":"30px","color":"#292929"},"blog_h5_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"400","font-style":"","subsets":"latin","text-align":"","text-transform":"","font-size":"14px","line-height":"24px","color":"#292929"},"blog_h6_font":{"font-family":"Montserrat","font-options":"","google":"1","font-weight":"400","font-style":"","subsets":"latin","text-align":"","text-transform":"","font-size":"12px","line-height":"18px","color":"#292929"},"blog_ahref_color":{"regular":"#292929","hover":"#1c46f2"},"blog_bg_color":"#ffffff","enable_css-code":"custom_css_off","css-code":"","enable_js-code":"custom_js_off","js-code":"jQuery(document).ready(function(){\r\n\r\n});","section_bk_to_top_btn":"","section_media_queries":"","section_containers":"","dark-scheme-info":"","light-scheme-info":"","section_page_title":"","section_menu_style":"","section_header_margins_paddings":"","section_header_style_one":"","section_complex_header_style":"","section_complex_header_style_top_bar":"","section_complex_header_style_middle_bar":"","section_complex_header_style_bottom_bar":"","footer_fallback":"","hgr_megafooter_support_info":"","cf_support_info":"","redux_options_object":"","redux_import_export":"","redux-backup":1}';

		update_option( 'redux_options', json_decode($json_content, true), '', 'yes' );
}


add_action('switch_theme', 'stash_theme_deactivated');
function stash_theme_deactivated () {
  delete_option('redux_options');
}


 // Portfolio Metaboxes
	function stash_portfoliometaboxes() {
    	$screens = array( 'hgr_testimonials' );
    	foreach ( $screens as $screen ) {
       		add_meta_box(
           	'hgr_testimetaboxid',
            	esc_html__( 'Testimonial details', 'stash' ),
            	'stash_testi_custom_box',
            	$screen
        	);
    	}
	}
	add_action( 'add_meta_boxes', 'stash_portfoliometaboxes' );
	function stash_testi_custom_box($post) {
		// Add an nonce field so we can check for it later
		wp_nonce_field( 'stash_testi_custom_box', 'stash_testi_custom_box_nonce' );
		...
		echo '<div class="settBlock"><label for="testi_author">';
		   esc_html_e( "Testimonial author", 'stash' );
		echo '</label> ';
		echo '<input type="text" id="testi_author" name="testi_author" value="' . esc_attr( $hgr_testi_author ) . '" size="25" placeholder="' . esc_html__( "Jon Doe", "stash" ) . '" /></div>';

	  	// Testimonial author company and job
	  	echo '<div class="settBlock"><label for="testi_role">';
		   esc_html_e( "Company and Position", 'stash' );
		echo '</label> ';
	  	echo '<input type="text" id="testi_role" name="testi_role" value="' . esc_attr( $hgr_testi_role ) . '" size="25" /></div>';
	}
	function stash_save_testidata( $post_id ) {
		...
		if ( isset($_POST['post_type']) && $_POST['post_type'] == 'hgr_testimonials' ) {
			...
			if ( !isset( $_POST['stash_testi_custom_box_nonce'] ) && ! wp_verify_nonce( $_POST['stash_testi_custom_box_nonce'] ) ) {
				return;
			}
	...
	add_action( 'save_post', 'stash_save_testidata' );
 ...
 // Pages Metaboxes
	...
	function stash_metaboxes() {
    	$screens = array( 'page', 'hgr_portfolio', 'hgr_megafooter' );
    	foreach ( $screens as $screen ) {
       		add_meta_box(
           	'hgr_metaboxid',
            	esc_html__( 'Page settings', 'stash' ),
            	'stash_inner_custom_box',
            	$screen
        	);
    	}
	}
	add_action( 'add_meta_boxes', 'stash_metaboxes' );
	// Print the box content
	function stash_inner_custom_box($post) {
		// Add an nonce field so we can check for it later
		wp_nonce_field( 'stash_inner_custom_box', 'stash_inner_custom_box_nonce' );
		...
		echo '<!--<div class="settBlock"><label for="page_color_scheme">';
		   esc_html_e( "Page color scheme", 'stash' );
		echo '</label> ';
		if($hgr_page_color_scheme == 'dark-scheme'){
			echo '<select name="page_color_scheme" id="page_color_scheme"><option value="dark-scheme" name="dark-scheme" selected="selected">'.esc_html__('Dark scheme', 'stash').'</option><option value="light-scheme" name="light-scheme">'.esc_html__('Light scheme', 'stash').'</option></select></div>-->';
		}
		elseif($hgr_page_color_scheme == 'light-scheme'){
			echo '<select name="page_color_scheme" id="page_color_scheme"><option value="dark-scheme" name="dark-scheme">'.esc_html__('Dark scheme', 'stash').'</option><option value="light-scheme" name="light-scheme" selected="selected">'.esc_html__('Light scheme', 'stash').'</option></select></div>-->';
		}
		else{
			echo '<select name="page_color_scheme" id="page_color_scheme"><option value="light-scheme" name="light-scheme" selected="selected">'.esc_html__('Light scheme', 'stash').'</option><option value="dark-scheme" name="dark-scheme">'.esc_html__('Dark scheme', 'stash').'</option></select></div>-->';
		}

		// Page background color
		echo '<div class="settBlock"><label for="page_bgcolor">';
		   esc_html_e( "Page background color", 'stash' );
		echo '</label> ';
		echo '<input type="text" id="page_bgcolor" name="page_bgcolor" class="color-field" value="' . $hgr_page_bgcolor . '" /></div>';
	  	...
	  	echo '<div class="settBlock"><label for="page_top_padding">';
		   esc_html_e( "Page top padding", 'stash' );
		echo '</label> ';
	  	...
	  	echo '<div class="settBlock"><label for="page_btm_padding">';
		   esc_html_e( "Page bottom padding", 'stash' );
	  	echo '</label> ';
		...
		// Page height
	  	echo '<div class="settBlock"><label for="page_height">';
		   esc_html_e( "Page height", 'stash' );
	  	echo '</label> ';
	  	echo '<input type="text" id="page_height" name="page_height" value="' . $hgr_page_height . '" size="25" /> <em>'.esc_html__('pixels. If not set, auto-height is set.', 'stash').'</em></div>';

		// Page title
	  	echo '<div class="settBlock"><label for="page_title">';
		   esc_html_e( "Disable page title", 'stash' );
	  	echo '</label> ';
		...
		echo '<div class="settBlock"><label for="page_title_color">';
		   esc_html_e( "Page title color", 'stash' );
		echo '</label> ';
		echo '<input type="text" id="page_title_color" name="page_title_color" class="color-field" value="' . $hgr_page_title_color . '" /> <em>'.esc_html__('Overrides the one set in Theme Options', 'stash').'</em></div>';
	}
	// Save the metabox data to database
	function stash_save_postdata( $post_id ) {
	...
	add_action( 'save_post', 'stash_save_postdata' );
 // END Pages Metaboxes
 ...
 */
 ...
 function stash_do_less($debug = false){
	...
	"container_over_xxl"		=>	esc_attr( $hgr_options['container_over_xxl']['width'] ),		// Default: 1400
	  ...
	  // Add To cart Button
	  "woo_addtocart_btn_color_regular"		=>	( isset($hgr_options['woo_addtocart_btn_color']['regular']) && !empty($hgr_options['woo_addtocart_btn_color']['regular']) ? esc_attr( $hgr_options['woo_addtocart_btn_color']['regular'] ) : '#000'),
	  "woo_addtocart_btn_color_hover"		=>	( isset($hgr_options['woo_addtocart_btn_color']['hover']) && !empty($hgr_options['woo_addtocart_btn_color']['hover']) ? esc_attr( $hgr_options['woo_addtocart_btn_color']['hover'] ) : '#999'),

		 // Add To cart Button Text
	  "woo_addtocart_btn_text_color_regular"=>	( isset($hgr_options['woo_addtocart_btn_text_color']['regular']) && !empty($hgr_options['woo_addtocart_btn_text_color']['regular']) ? esc_attr( $hgr_options['woo_addtocart_btn_text_color']['regular'] ) : '#000'),
	  "woo_addtocart_btn_text_color_hover"	=>	( isset($hgr_options['woo_addtocart_btn_text_color']['hover']) && !empty($hgr_options['woo_addtocart_btn_text_color']['hover']) ? esc_attr( $hgr_options['woo_addtocart_btn_text_color']['hover'] ) : '#999'),

	  "qcv_button_checkout_regular_color"	=>	( isset($hgr_options['qcv_chk_btn_bg_color']['regular']) && !empty($hgr_options['qcv_chk_btn_bg_color']['regular']) ? esc_attr( $hgr_options['qcv_chk_btn_bg_color']['regular'] ) : '#fff'),
	  "qcv_button_checkout_hover_color"		=>	( isset($hgr_options['qcv_chk_btn_bg_color']['hover']) && !empty($hgr_options['qcv_chk_btn_bg_color']['hover']) ? esc_attr( $hgr_options['qcv_chk_btn_bg_color']['hover'] ) : '#333'),
	  ...
	  "back_to_top_button_width"			=>	( isset($hgr_options['back_to_top_button_dimensions']['width']) && !empty($hgr_options['back_to_top_button_dimensions']['width']) ? esc_attr( $hgr_options['back_to_top_button_dimensions']['width'] ) : '30px'),
		...
		* Blog type metro
		*/
		"metro_columns_gap"			=>	( isset($hgr_options['blog_metro_columns_gap']['width']) && !empty($hgr_options['blog_metro_columns_gap']['width']) ? esc_attr( $hgr_options['blog_metro_columns_gap']['width'] ) : '0'),
	...
	$products_per_row	=	( isset($hgr_options['products_per_row']) && !empty($hgr_options['products_per_row']) ? esc_attr( $hgr_options['products_per_row'] ) : '3' );
	 ...
	 $page_light_ahref_color_regular	=	esc_attr( $hgr_options['light_ahref_color']['regular'] );
	$page_light_ahref_color_hover		=	esc_attr( $hgr_options['light_ahref_color']['hover'] );
	$page_dark_ahref_color_regular		=	esc_attr( $hgr_options['ahref_color']['regular'] );
	$page_dark_ahref_color_hover		=	esc_attr( $hgr_options['ahref_color']['hover'] );

	if ( class_exists( 'ReduxFramework' ) ) {
		$rgba_theme_dominant_color = Redux_Helpers::hex2rgba($theme_dominant_color);
		$rgba_page_light_ahref_color_hover = Redux_Helpers::hex2rgba($page_light_ahref_color_hover);
		$rgba_page_dark_ahref_color_hover = Redux_Helpers::hex2rgba($page_dark_ahref_color_hover);
		$rgba_fixed_menu_hover_color = Redux_Helpers::hex2rgba($fixed_menu_hover_color);
	} else {
		$rgba_theme_dominant_color = "234,56,78";
		$rgba_page_light_ahref_color_hover = "123,42,56";
		$rgba_page_dark_ahref_color_hover = "111,22,44";
		$rgba_fixed_menu_hover_color = "250,50,67";
	}
	...
	.lity{
		background: ".$theme_dominant_color.";
		background: rgba(". $rgba_theme_dominant_color ." , 0.9);
	}




	.light-scheme a.link-curtain::before {
		border-top: 2px solid rgba(". $rgba_page_light_ahref_color_hover ." , 1);
	}
	.light-scheme a.link-curtain::after {
		background: rgba(". $rgba_page_light_ahref_color_hover ." , 0.1);
	}

	.dark-scheme a.link-curtain::before {
		border-top: 2px solid rgba(". $rgba_page_dark_ahref_color_hover ." , 1);
	}
	.dark-scheme a.link-curtain::after {
		background: rgba(". $rgba_page_dark_ahref_color_hover ." , 0.1);
	}

	#hgr_top_navbar_container .dropdown-menu li a:hover{
		background-color: rgba(". $rgba_theme_dominant_color ." , 0.10);
	}

	#fixed_navbar .dropdown-menu li a:hover{
		background-color: rgba(". $rgba_fixed_menu_hover_color ." , 0.15);
	}
	...
	.fakeMobile #hgr_top_navbar_container .dropdown-menu,
	.isMobile.isDesktop #hgr_top_navbar_container .dropdown-menu{
		border-top: 0 !important;
	}

	.fakeMobile #hgr_top_navbar_container .dropdown-menu li.dropdown-submenu ul li,
	.isMobile.isDesktop #hgr_top_navbar_container .dropdown-menu li.dropdown-submenu ul li{
		padding-left:20px;
	}
	...
	.shop_widget ul li {
		border-left: solid 6px rgba(". $rgba_theme_dominant_color ." , 0.3);
	...
	.shop_widget ul li:hover {
		...
		background-color: rgba(". $rgba_theme_dominant_color ." , 0.05);
	}
	...
	.page-template-page-rightsidebar #wp-calendar thead {
		background: rgba(". $rgba_theme_dominant_color ." , 0.05);
	}
	.page-template-page-leftsidebar #wp-calendar tbody td:hover,
	.page-template-page-rightsidebar #wp-calendar tbody td:hover {
		background: rgba(". $rgba_theme_dominant_color ." , 0.08);
	}


	.price_slider_wrapper .ui-slider-horizontal {
		background-color: rgba(". $rgba_theme_dominant_color ." , 0.2);
	}
	.price_slider_wrapper .ui-slider .ui-slider-range,
	.price_slider_wrapper .ui-slider .ui-slider-handle {
		background: rgba(". $rgba_theme_dominant_color ." , 1);
	}
	/*.woocommerce #respond input#submit,
	.woocommerce a.button,
	.woocommerce button.button,
	.woocommerce input.button{
		background-color: rgba(". $rgba_theme_dominant_color ." , 1)!important;
	}
	.woocommerce #respond input#submit:hover,
	.woocommerce a.button:hover,
	.woocommerce button.button:hover,
	.woocommerce input.button:hover{
		background-color: rgba(". $rgba_theme_dominant_color ." , 0.5)!important;
	}*/
	...
	*/
	...
	.woocommerce-page div.product form.cart .button {background-color: @woo_addtocart_btn_color_regular;padding-left: 30px !important;padding-right: 30px !important;}

	.woocommerce #content div.product form.cart .button:hover,
	.woocommerce div.product form.cart .button:hover,
	.woocommerce-page #content div.product form.cart .button:hover,
	.woocommerce-page div.product form.cart .button:hover {background-color: @woo_addtocart_btn_color_hover;padding-left: 30px !important;padding-right: 30px !important;}

	.woocommerce #respond input#submit.alt, .woocommerce a.button.alt, .woocommerce button.button.alt, .woocommerce input.button.alt {color: @woo_addtocart_btn_text_color_regular;}
	.woocommerce #respond input#submit.alt, .woocommerce a.button.alt, .woocommerce button.button.alt, .woocommerce input.button.alt:hover {color: @woo_addtocart_btn_text_color_hover;}
	...
	.fakeMobile #hgr_left, .fakeMobile #hgr_right {width: 0;}
	.fakeMobile #hgr_top, .fakeMobile #hgr_bottom {height: 0;}
	body.fakeMobile  {margin-top: 0 !important;}
	.fakeMobile .bka_footer{margin-bottom: 0 !important;}
	...
	.blogGridItem .post_format-post-format-quote{
		background-color: rgba(". $rgba_theme_dominant_color ." , 0.8)!important;
		color:#fff;
	}
	.blogGridItem .post_format-post-format-quote .fa{
		color: ".$theme_dominant_color.";
		margin-top:0;
	}

	#metroGridItems .postEntry{
		margin:@metro_columns_gap/2!important;
		padding:10px!important;
		background-color: rgba(". $rgba_theme_dominant_color ." , 1);
	}
...
//			text-align:center;
	...
	/*
		...
		Under:		mediaquery_screen_xxl
		Default:	1280px
		Container:	container_xl
		Media:		(min-width: 981px) and (max-width: 1280px)
	*/
	@media (min-width: @mediaquery_screen_xl) and (max-width: @mediaquery_screen_xxl - 1)  {
...
//			text-align:center;
	...
	/*
		Over XXL Screen
		Over:		mediaquery_screen_xxl
		Under:		none
		Default:	over 1280px
		Container:	container_xxl
		Media:		min-width: 1401px
	*/
	@media (min-width: @mediaquery_screen_xxl) {
		.container, #container{
			max-width: @container_over_xxl;
		}
		...
		".( isset($hgr_options['website_model']) && $hgr_options['website_model'] === 'website_boxed' ?
		' #masthead, #website_boxed, #hgr_top_navbar_container {width: @container_over_xxl!important;} .wpb_row{padding-left:50px;padding-right:50px;}' : ' #website_boxed{width: 100vw;max-width:100%;} ')."
		".( isset($hgr_options['enable_boxed_shadow']) && $hgr_options['enable_boxed_shadow'] == '1' ?
		' #website_boxed  {-webkit-box-shadow: 0px 0px 60px 0px rgba(0,0,0,0.2);-moz-box-shadow: 0px 0px 60px 0px rgba(0,0,0,0.2);box-shadow: 0px 0px 60px 0px rgba(0,0,0,0.2);} ' : '')."
...
//			text-align:center;
	...
	wp_add_inline_style( 'stash_custom-styles', esc_html($less->compile($options)) );
 }
 add_action( 'wp_enqueue_scripts', 'stash_do_less' );
...
*/
function stash_woo_show_product_atributes(){
...
*/
function stash_get_layout( $page_type = 'page' ){
	$hgr_options = get_option( 'redux_options' );
...
function stash_do_header() {
	...
	*/
	...
	switch($header_type){
		case '2': // Apear after scrolling down
			...
			$output = 'jQuery(document).ready(function($) {
					...
					var lastScrollTop = 0;
					$(".hgr_woo_minicart").css("min-height", "60px" );
					...
					if($(window).height() < $("body").prop("scrollHeight")) {
							...
							$(window).bind("scroll", function() {
								...
								if ($(window).scrollTop() > '.$scroll_amount.') {
									$("#hgr_top_navbar_container").removeClass("hidden").removeClass("headerhidden").addClass("headerappear");
								}
								if ($(window).scrollTop() < '.$scroll_amount.') {
	...
	wp_add_inline_script( 'stash_js', $output, 'after' );

	}
	add_action( 'wp_enqueue_scripts', 'stash_do_header' );


 // Custom search form
 function stash_search_form( $form ) {
    $form = '<form method="get" id="searchform" class="searchform" action="' . esc_url( home_url( '/' ) ) . '" >
    <div>
    <input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="'. esc_html__( 'Search','stash' ) .'" />
    <input type="submit" id="searchsubmit" value="'. esc_html__( 'Search','stash' ) .'" />
    </div>
    </form>';

    return $form;
 }
 add_filter( 'get_search_form', 'stash_search_form' );





 // Include the Redux Framework config file
...
// Custom CSS for Highgrade Framework admin panel
	function stash_addAndOverridePanelCSS() {
	...
	add_action( 'redux/page/hgr_options/enqueue', 'stash_addAndOverridePanelCSS' );

	add_action('admin_head', 'stash_custom_meta_css');
	function stash_custom_meta_css() {
	  echo '<style>
		#hgr_metaboxid label {
	...
	function stash_get_post_meta_by_key() {
		global $wpdb;
		$vc_styles = '';
		...
		if ( !empty($meta) ) {
			...
			wp_add_inline_style( 'stash_custom-styles', esc_attr($vc_styles) );
		}
		return false;
	}
	add_action( 'wp_enqueue_scripts', 'stash_get_post_meta_by_key' );


	function stash_get_custom_css() {
		$hgr_options = get_option( 'redux_options' );
		if ( isset($hgr_options['enable_css-code']) && $hgr_options['enable_css-code'] == 'custom_css_on') {
			if( !empty($hgr_options['css-code']) ){
				wp_add_inline_style( 'stash_custom-styles', esc_attr($hgr_options['css-code']) );
			}
		}
		return false;
	}
	add_action( 'wp_enqueue_scripts', 'stash_get_custom_css' );
...
* HGR Menu Fallback
*/
function stash_menu_fallback(){
	echo '<ul id="mainNavUl" class="nav navbar-nav navbar-right"><li class="menu-item menu-item-type-post_type menu-item-object-page current-menu-item page_item page-item-480 current_page_item"><a href="' . esc_url( home_url( '/' ) ) . '">'. esc_html__( 'Home','stash' ) .'</a></li></ul>';
}
...
add_action( 'admin_menu', 'stash_remove_redux_menu',12 );
function stash_remove_redux_menu() {
    remove_submenu_page('tools.php','redux-about');
}

add_action( 'admin_bar_menu', 'stash_remove_element_from_adminbar', 999 );
function stash_remove_element_from_adminbar( $wp_admin_bar ) {
	$wp_admin_bar->remove_node( 'theme_options' );
}


function stash_remove_redux_demo_links() {
...
add_action('init', 'stash_remove_redux_demo_links');
...
*/
 function stash_styles(){
	...
	wp_add_inline_style( 'stash_custom-styles', esc_attr($output) );
 }
 add_action( 'wp_enqueue_scripts', 'stash_styles' );
 ...
 **/
 add_filter( 'wc_add_to_cart_message_html', 'stash_custom_add_to_cart_message' );
 function stash_custom_add_to_cart_message() {
	global $woocommerce;
	...
	$message = sprintf('<a href="%s" class="button">%s</a> %s', $return_to, esc_html__('Continue Shopping &rarr;', 'stash'), esc_html__('Product successfully added to your cart.', 'stash') );

	else :

	$message = '<i class="fa fa-icon fa-check hgr_cart_msg"></i>';

	$message .= sprintf('<a href="%s" class="hgr_view_cart_link">%s <i class="fa fa-icon fa-angle-right hgr_view_cart"></i></a> %s', get_permalink( wc_get_page_id('cart') ), esc_html__('View Cart', 'stash'), esc_html__('Product successfully added to your cart.', 'stash') );
...
if (is_admin()) {
	require_once locate_template('/highgrade/hgr_vc_templates/hgr_vc_templates.php');
	add_action('admin_enqueue_scripts', 'hgr_vc_templates_styles');
	function hgr_vc_templates_styles() {
		wp_enqueue_style('hgr_vc_templates_styles', trailingslashit( get_template_directory_uri() ) . 'highgrade/hgr_vc_templates/assets/hgr_vc_templates.css', array(), '', '');
		wp_enqueue_script( 'hgr_vc_templates_js',	trailingslashit( get_template_directory_uri() ) . 'highgrade/hgr_vc_templates/assets/hgr_vc_templates.js', array(), '',false );
	}
}
...
*	Typekit fonts
*/
function stash_typekit_js(){
	$hgr_options = get_option( 'redux_options' );
	if( !empty($hgr_options['typekit_embed_code']) && !empty($hgr_options['typekit_custom_css']) ){
		echo ($hgr_options['typekit_embed_code']);
	}
}
add_action('wp_head', 'stash_typekit_js');
function stash_typekit_css(){
	$hgr_options = get_option( 'redux_options' );
	if( !empty($hgr_options['typekit_embed_code']) && !empty($hgr_options['typekit_custom_css']) ){
		wp_add_inline_style( 'stash_custom-styles', $hgr_options['typekit_custom_css'] );
	}
}
add_action( 'wp_enqueue_scripts', 'stash_typekit_css' );


/*
*	Portfolio layout display
*/
if( !function_exists('stash_portfolio_layout_display') ) {
	function stash_portfolio_layout_display() {
		$hgr_options = get_option( 'redux_options' );

		$portfolio_sidebar = ( isset($hgr_options['portfolio_sidebar']) && !empty($hgr_options['portfolio_sidebar']) ? $hgr_options['portfolio_sidebar'] : 'sidebar_none');

		get_template_part( 'layouts/portfolio/portfolio_' . $portfolio_sidebar );
	}
	add_action('portfolio_get_layout', 'stash_portfolio_layout_display', 5);
}

/*
*	Portfolio Content display by type
*/
if( !function_exists('stash_display_portfolio_content') ) {
	function stash_display_portfolio_content() {
		$hgr_options = get_option( 'redux_options' );

		$portfolio_type = ( isset($hgr_options['portfolio_type']) && !empty($hgr_options['portfolio_type']) ? $hgr_options['portfolio_type'] : 'portfolio_grid');

		get_template_part( 'layouts/portfolio/' . $portfolio_type );
	}
	add_action('portfolio_get_content', 'stash_display_portfolio_content', 5);
}
...
*	Blog layout display
*/
if( !function_exists('stash_blog_layout_display') ) {
	function stash_blog_layout_display() {
		$hgr_options = get_option( 'redux_options' );

		$blog_sidebar = ( isset($hgr_options['blog_sidebar']) && !empty($hgr_options['blog_sidebar']) ? $hgr_options['blog_sidebar'] : 'sidebar_none');

		get_template_part( 'layouts/blog/blog_' . $blog_sidebar );
	}
	add_action('blog_get_layout', 'stash_blog_layout_display', 5);
}


/*
*	Blog Content display by type
*/
if( !function_exists('stash_display_blog_content') ) {
	function stash_display_blog_content() {
		$hgr_options = get_option( 'redux_options' );

		$blog_type = ( isset($hgr_options['blog_type']) && !empty($hgr_options['blog_type']) ? $hgr_options['blog_type'] : 'blog_basic');

		get_template_part( 'layouts/blog/' . $blog_type );
	}
	add_action('blog_get_content', 'stash_display_blog_content', 5);
}
