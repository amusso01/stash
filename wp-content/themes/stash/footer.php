...
elseif( class_exists('HGR_MEGAFOOTER') && empty($hgr_megafooterID) && isset($hgr_options['hgr_megafooter_select']) && !empty($hgr_options['hgr_megafooter_select']) ){
	...
	wp_reset_postdata();
	echo '<div class="container hgr_megafooter">';
	echo do_shortcode($hgr_megafooter['post_content']);
	echo '<div class="clearfix"></div></div>';
}
elseif( class_exists('HGR_MEGAFOOTER') && $hgr_megafooterID == "minimal_footer" ){ ?>
<div class="clearfix"></div>
       <div class="row bka_footer minimal_footer " style=" <?php echo( !empty($hgr_options['footer-bgcolor']) ? ' background-color:' . esc_attr( $hgr_options['footer-bgcolor'] ) . ';' : '');?>">
        <div class="container">
            <div class="vc_col-md-12">
                <?php echo ( !empty($hgr_options['footer-copyright']) ? wp_kses( $hgr_options['footer-copyright'], $allowed_html_array ) : esc_html__('Set your Copyright Text into Theme Options', 'stash') );?>
            </div>
            <div class="clearfix"></div>
        </div>
    </div>
...
elseif( class_exists('HGR_MEGAFOOTER') && $hgr_megafooterID == "no_footer" ){
	...
	?>
    <div class="clearfix"></div>
       <div class="row bka_footer minimal_footer <?php echo( !empty($hgr_options['footer_color_scheme']) ? esc_attr( $hgr_options['footer_color_scheme'] ) : ' dark-scheme ');?>" style=" <?php echo( !empty($hgr_options['footer-bgcolor']) ? ' background-color:' . esc_attr( $hgr_options['footer-bgcolor'] ) . ';' : '');?>">
        <div class="container">
            <div class="vc_col-md-12">
                <?php echo ( !empty($hgr_options['footer-copyright']) ? wp_kses( $hgr_options['footer-copyright'], $allowed_html_array ) : esc_html__('Set your Copyright Text into Theme Options', 'stash') );?>
            </div>
